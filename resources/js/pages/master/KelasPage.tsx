...
const columns: Column<Kelas>[] = [
    ...
    { key: "semester", label: "Semester", cellAlign: "center" },
...
export default function KelasPage() {
    ...
    const handleModalOpen = (row: Kelas | null, setFormState: React.Dispatch<React.SetStateAction<Record<string, string>>>) => {
    ...
    const formFields = useMemo<FieldDef[]>(() => {
        const fields: FieldDef[] = [
            { key: "nama_kelas", label: "Nama Kelas", required: true, placeholder: "Contoh: S1 TE - A" },
        ...
        return fields;
    }, [isJurusanRestricted, jurusanOptions, prodiOptions, semesterOptions]);
    ...
    const filterOptions = useMemo<FilterOption[]>(() => {
        ...
        filters.push({ label: "Semua Semester", key: "semester", options: semesterOptions });
        return filters;
    }, [isJurusanRestricted, jurusanOptions, prodiOptions, semesterOptions]);
    ...
    return (
        <MasterDataPage<Kelas>
            title="Data Kelas"
            description={`Manajemen data kelas rombongan belajar (Semester ${semesterTipe === 'ganjil' ? 'Ganjil' : 'Genap'})`}
            apiEndpoint={`/kelas?semester_tipe=${semesterTipe}`}
            columns={columns}
            ...
            importUrl={canEditData ? "/kelas/import" : undefined}
            templateUrl={canEditData ? "/template/template-import-kelas.xlsx" : undefined}
        />
    );
}
